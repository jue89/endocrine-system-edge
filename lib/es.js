...
class EndocrineSystem extends events.EventEmitter {
	...
	constructor( options ) {
		...
		// Test all provided PKI data and the time drift of the local clock
		this._brokerPromise = Promise.all( [
			...
			broker.on( 'message', ( topic, message ) => {
				...
				for( let s in this._subscriptions ) {
					...
					if( sub.topicRE( topic ) ) sub.handler( topic, message.toString() );
				}
			} );
			// - Connection
			...
			// - Reconnection
			//   The connection has been reestablished. The subscriptions are still
			//   known to the broker.
			// NOPE: Löst aus, wenn er ein reconnect versucht
			broker.on( 'reconnect', () => {
				// Tell the user we are online again
				this.emit( 'online' );
			} );
			// - Close
			...
			// - Disconnect
			//   Fired if we disconnection from the broker
