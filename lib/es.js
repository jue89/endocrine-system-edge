...
class EndocrineSystem extends events.EventEmitter {
	...
	constructor( options ) {
		...
		this._brokerPromise = Promise.all( [
			...
			// If no broker has been give search in the local network via mdns
			let service = 'es-' + result[ 3 ].replace( /\:/g, '' ).substr( 0, 7 );
			return discover.mDNS( service ).then( ( server ) => {
				url = 'mqtts://' + server.host + ':' + server.port.toString();
			} );
