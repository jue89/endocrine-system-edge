...
class EndocrineSystem extends events.EventEmitter {
	...
	_processOptions( options ) {
		...
		options.core.forEach( ( item ) => {
			...
			if( type != 'string' && type != 'function' ) {
				throw new Error( "Invalid core discovery method. Allow types: string, function returning promise." );
			}
		} );
	...
	_discover() {
		...
		// Get fingerprint for discovery
		return pem.getFingerprint( this._ca ).then( ( fingerprint ) => {

			// Kick off discovery
			return discover( fingerprint, this._coreDiscovery, 0 );
