...
class Sink extends Events {
	...
	constructor( es, filter, certCheck ) {
		...
		// Create some stores ...
		this._subscriptions = {}; // Subscription handles to reveived definitions
		this._definitions = {};   // Received definitions
	...
	shutdown() {
		...
		return this._es._unsubscribe( this._subHandle ).then( () => {
	...
	_addHormoneSource( name, payload ) {
		...
		return new Promise( ( resolve ) => {
			...
			if( this._definitions[ name ] ) {
				...
				let former = this._definitions[ name ].data;
			...
			if( ! this._definitions[ name ] ) {
				...
				return this._es._subscribe( 'hormone/' + name, ( topic, payload ) => {
					...
					this._subscriptions[ name ] = subHandle;
	...
	_removeHormoneSource( name ) {
		...
		// Unsubscribe hormone
		return this._es._unsubscribe( this._subscriptions[ name ] ).then( () => {
			...
			// Emit an event
			return this._emitDefinitionEvent( 'undefined', name, this._definitions[ name ] );

		} ).then( () => {

			// Remove subscription, definition and last hormone
	...
	_receivedHormone( topic, payload ) {
		...
		try {
			...
			if( ! this._definitions[ name ] ) throw new Error( "Received unkown hormone" );

			// Check if we received a duplicate
			if( this._hormones[ name ] && this._hormones[ name ].hormone.payload == payload ) return;

			// Interprete hormone
			let hormone = new HormoneSink( this._definitions[ name ], payload );
			...
			if( isFresh ) timeout = setTimeout( () => {
				...
				this._hormones[ name ].stateChangedAt = time.now();
	...
	_fetchHormones( cond ) {
		...
		for( let name in this._hormones ) {
			...
			if( cond.isOK !== undefined && h.isOK !== cond.isOK ) {
				add = false;
			}
			...
			if( add ) hormones.push( {
				...
				isOK: h.hormone.isOK,
				error: h.hormone.error,
				isFresh: h.hormone.isFresh,
				freshness: h.hormone.freshness
			} );
	...
	get erroneousHormones() { return this._fetchHormones( { isOK: false } ); }
	get goodHormones() { return this._fetchHormones( { isOK: true, isFresh: true } ); }
