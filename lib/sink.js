...
class Sink extends events.EventEmitter {
	...
	_addHormoneSource( name, payload ) {
		...
		return new Promise( ( resolve ) => {
			...
			if( ! this._subscriptions[ name ] ) {
				...
				this.emit( 'refreshed', name, definition );
	...
	_receivedHormone( topic, payload ) {
		try {
			...
			if( isFresh && ! formerHormone.isFresh ) {
				...
				setImmediate( () => this.emit( 'hormoneRefresh', name, hormone ) );
				stateChanged = true;
			} else if( ! isFresh && formerHormone.isFresh ) {
				// The received hormone has expired!
				setImmediate( () => this.emit( 'hormoneExpiration', name, hormone ) );
				stateChanged = true;
			}
			// - Error
			...
			if( err === 0 && formerHormone.err != err ) {
				...
				setImmediate( () => this.emit( 'hormoneRecovery', name, hormone ) );
				stateChanged = true;
			} else if( err > 0 && formerHormone.err != err ) {
				// Some error occured!
				setImmediate( () => this.emit( 'hormoneError', name, hormone ) );
				stateChanged = true;
			}
			...
			if( isFresh ) timeout = setTimeout( () => {
				...
				this.emit( 'hormoneExpiration', name, hormone );
	...
	get erroneousHormones() { return this._fetchHormones( { err: true } ); }
