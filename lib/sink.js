...
class Sink extends events.EventEmitter {
	...
	destroy() {
		...
		return this._es._unsubscribe( 'defintion/' + this._filter ).then( () => {
	...
	_addHormoneSource( name, payload ) {
		...
		return new Promise( ( resolve ) => {
			...
			let now = time.now();
			if( results[1].validity.start / 1000 > now ) {
				throw new Error( "Certificate is valid in the future" );
			}
			if( results[1].validity.end / 1000 < now ) {
			...
			if( this._subscriptions[ name ] ) this._removeHormoneSource( name );
