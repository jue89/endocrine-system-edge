...
class HormoneSink extends Hormone {
	...
	constructor( definition, payload ) {
		...
		for( let i = 0; i < format.length; i++ ) {
			...
			switch( f.type ) {
				case 'number':  data[ f.name ] = parseFloat( rawData[i] ); break;
				case 'string':  data[ f.name ] = rawData[i].toString(); break;
				case 'boolean': data[ f.name ] = ( rawData[i] === '1' ) ? true : false; break;
			}
